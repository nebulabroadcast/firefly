...
def import_template(scheduler, day_offset=0):
    ...
    try:
        ...
        log.traceback()
        return
    ch, cm = scheduler.calendar.day_start
    events = []
    try:
        for day_index, day in enumerate(data.findall("day")):
            day_start = scheduler.calendar.week_start_time + (3600 * 24 * day_index)
        ...
        QApplication.restoreOverrideCursor()
        log.traceback("Unable to parse template:")
        return
    if not events:
        QApplication.restoreOverrideCursor()
        return
    ...
    response = api.scheduler(
        id_channel=scheduler.id_channel, date=scheduler.calendar.date, events=events
    )
    QApplication.restoreOverrideCursor()
    if not response:
        ...
        scheduler.calendar.set_data(response["events"])
