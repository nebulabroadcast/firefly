...
_handle_gl_func(
    "mpv_opengl_cb_init_gl", [c_char_p, OpenGlCbGetProcAddrFn, c_void_p], c_int
)
...
class MPV(object):
    ...
    def terminate(self):
        ...
        if threading.current_thread() is self._event_thread:
            ...
            grim_reaper = threading.Thread(
                target=lambda: _mpv_terminate_destroy(handle)
            )
            grim_reaper.start()
        else:
    ...
    def event_callback(self, *event_types):
        ...
        def register(callback):
            types = [
                MpvEventID.from_str(t) if isinstance(t, str) else t for t in event_types
            ] or MpvEventID.ANY
